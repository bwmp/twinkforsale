...
export const DetailedAnalyticsChart = component$<DetailedAnalyticsChartProps>(
  ({ data, metric, height = 200, color = "var(--theme-primary)" }) => {
    if (!data || data.length === 0) {
      return (
        <div 
          class="flex items-center justify-center bg-theme-card-border/20 rounded-lg"
    ...
    const pathPoints = data.map((d, index) => {
      ...
      const y = padding + (1 - ((d[metric] || 0) - minValue) / (maxValue - minValue)) * innerHeight;
      return `${x},${y}`;
    }).join(' ');
    ...
    return (
      <div class="w-full">
        ...
        >
          ...
          {data.map((d, index) => {
            ...
            return (
              <circle
                ...
                r="3"
                fill={color}
                class="hover:r-5 transition-all duration-200"
              >
          ...
          <g class="text-theme-text-muted" style="font-size: 12px;">
            {Array.from({ length: numGridLines + 1 }, (_, i) => {
              const value = minValue + (maxValue - minValue) * (1 - i / numGridLines);
              const y = padding + (i / numGridLines) * innerHeight;
        ...
        <div class="mt-4 flex justify-between text-sm">
          ...
          <div class="text-theme-text-secondary">
            <span class="font-medium">Total:</span> {values.reduce((a, b) => a + b, 0)}
          </div>
        </div>
      </div>
    );
