...
export const DiscordProfile = component$<DiscordProfileProps>(
  ({ discordId, config = {}, class: className = "" }) => {
    ...
    if (
      ...
      return (
        <div class={`rounded-lg bg-red-500/10 p-4 text-red-400 ${className}`}>
          ...
          {error.value && <p class="text-xs text-red-300">{error.value}</p>}
        </div>
      );
    }
    ...
    const statusText = getStatusText(data.discord_status); // Get all activities excluding Spotify and custom status
