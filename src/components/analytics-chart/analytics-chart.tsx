...
export const AnalyticsChart = component$<AnalyticsChartProps>(({ data, metric, title, color }) => {
  if (!data || data.length === 0) {
    return (
      <div class="text-center py-8">
        <div class="text-pink-300 mb-2">📈</div>
        <p class="text-pink-200 text-sm">No data available</p>
      </div>
    );
  }
  ...
  return (
    <div class="card-cute p-4 rounded-2xl">
      <h3 class="text-sm font-medium text-pink-200 mb-3 flex items-center gap-2">
      ...
      <svg width={chartWidth} height={chartHeight} class="w-full">
        ...
        {/* Y-axis labels */}
        <text x={padding - 5} y={padding + 5} text-anchor="end" class="text-xs fill-pink-300">
          {maxValue}
        </text>
        <text x={padding - 5} y={padding + innerHeight + 5} text-anchor="end" class="text-xs fill-pink-300">
          0
        </text>
      </svg>
      ...
      <div class="mt-2 text-center">
        ...
        <span class="text-xs text-pink-300 ml-1">today</span>
      </div>
    </div>
  );
});
