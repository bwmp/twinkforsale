...
/**
 * Gets analytics data for the last N days
 */
...
/**
 * Gets view analytics for a specific upload over the last N days
 */
...
/**
 * Gets analytics data for a specific user over the last N days
 */
export async function getUserAnalytics(userId: string, days: number = 7) {
  ...
  // Get view logs for all user's uploads
  const viewLogs = await db.viewLog.findMany({
    where: {
  ...
  // Get uploads created by this user in the time period
